...
Our main result is to present a new architecture that allows to efficiently train a CNN with the minimal memory cost of XXXbytes per pixel.
We demonstrate the efficiency of our method by efficiently training a model to 94\% accuracy on the CIFAR10 dataset within XXX minutes on a low-end Nvidia GTX750 with only 1GB of VRAM.
...
requires $\mathcal{M}=XXX$GB of VRAM for a training iteration over batch of 512 images of resolution $32 \times 32$.
...
We detail the computational cost of different reversible architectures in section XXX. 
...
Figure XXX illustrates the evolution of the SNR degradation factor $\alpha$ for different $\rho$.
...
Figure XXX shows the evolution of the SNR degradation $\alpha$ for different negative slopes $n$; 
and, in Section XXX, we investigate the impact of the negative slope parameter on the model accuracy.
...
In XXX \cite{jacobsen2018revnet}, the authors propose an inversible pooling operation that operates 
by stacking the neighboring elements of the pooling regions along the channel dimension.
AS noted in Section XXX, the increase in channel size at each pooling level induces a quadratic increase in the following convolution layers parameters.
...
The peak memory usage for an iteration of taining this architecture can be computed as follows:
...
In section XXX, we will quantify the accumulation of numerical errors through long chains of layer-wise invertible operations and show that numerical errors negatively impact model accuracy.

To prevent these numerical instabilities, we introduce an hybrid architecture, illustrated in Figure XXX, combining reversible residual blocks with layer-wise invertible functions.
...
The backward pass through these hybrid reversible blocks is illustrated in Figure XXX and proceeds as follows: 
...
However, this architecture adds an extra computational cost as both the reversible block inverse and layer-wise inverse need to be computed.
...
In this section, we quantify the accumulation of numerical errors in layer-wise invertible architectures (illustrated in Figure XXX) and analyse their impact on the accuracy.
Figure XXX illustrates the evolution of the SNR along the layers of a toy XXX-layer model.
This figure illustrates the continuous degradation of the reconstructed signal as errors accumulate within each layer.

We found the depth $N$ of the network and the negative slope $n$ of the activation function to be the two factors impacting numerical errors the most.
Figure XXX shows the evolution of numerical errors for network of different depth and different negative slopes.
We are interested in the impact these numerical errors have on the accuracy.
In order to isolate the numerical errors impact from other hyper-parameters,, 
we compare the accuracy reached by a same architecture with and without noisy reconstructions.
We investigate architectures with different parameters and, for each architecture, we compute the accuracy reached when 
hidden activations are reconstructed during the backward pass to the accuracy reached using the original hidden activations,
as computed during the forward pass.

The impact of numerical errors is given by the difference between both results, in Figure XXX.
In this figure, we compare the accuracy reached for different depth $N$ and negative slopes $n$.
For small depths and high negative slopes settings, 
in which the numerical errors are minimum, 
both models yield similar accuracy.
...
illustrated in Figure XXX, to prevent the negative impact of numerical errors on accuracy.
Figure XXX shows the propagation of the signal to noise ratio through the layers of such hybrid architecture.
While the hybrid architecture also suffers from a sharp SNR degradation within the reversible blocks,
we can see that the SNR almost perfectly recovers at the end of each reversible block.
This illustrates the benefits of using the reversible block inverse instead of layer-wise inversion to 
limit the propagation of reconstruction errors to lower layers.

Figure XXX reproduces the experiment of Figure XXX with an hybrid architecture.
As the hybrid architecture stabilizes training, the negative impact of numerical errors 
observed in the layer-wise architecture are gone.

\subsection{Model comparison}

Table 1 summarizes our main result.
In this table, we compare architectures with different patterns of reversibility.
To allow for a fair comparison, we have tweaked each architecture to keep their 
number of parameter as close as possible.
...
Nvidia GTX750.
The GTX750 only has 1GB of VRAM, which results in roughly 500MB of available memory after the initialization of various frameworks.
...
and illustrated the efficiency of our approach by training a CNN to 94\% accuracy on a low-end GPU within only XXX of memory.
...
\caption{
Illustration of a hybrid architecture and its memory consumption. 
XXX
}
\end{figure}
...
\caption{
Evolution of the SNR through the layers of a XXX-layers layer-wise invertible model.
The SNR gets continuously degraded throughout each layer
}
\end{figure}
...
\caption{
Illustration of the numerical errors and accuracy of a layer-wise invertible model.
}
\end{figure}
...
\includegraphics{Figure12.png}
\caption{
Illustration of the numerical errors and accuracy of a hybrid architecture model.
}
\end{figure}
\end{backmatter}
\end{document}
