...
\author[
   addressref={aff3},
   noteref={n1},
   email={quentin.febvre@gmail.com}
...
The memory cost of the hidden activations is thus the main memory bottleneck of CNN training as the cost assoiated with 
...
To validate our formula, we emperically evaluate $\alpha$ for normal Gaussian inputs $x$ and output noise $\epsilon^y$ and find it to closely match the theoretical results given by equation 19. 
...
Figure 5 plots the emprically found $\alpha$ values against the theoretical values predicted by our formula and show a close match. 
...
the other architecture we evalutate instead of the single module used in the original architecture.
...
We would be happy to thank reviewers for useful feedbacks.
...
At this step of the computations, all hidden activations within the reversible block are stored in memory simulateously.
}
\end{figure}
...
(Right) Comparison of the theoretical and empricial evolution of the $\alpha$ ratio for different for arbitrarily parameterized layers.
Emrpirical values were computed for random Gaussian input signal and a white gaussian noise of standard deviation $10^{-5}$.
}
\end{figure}
...
\caption{
Illustration of the impact of depth (in number of layers $N$) and negative slope $n$ on the numerical errors.
Both figure shows the evolution of the SNR at the lowest layer of our hybriud architecture with increasing depth and negative slopes.
